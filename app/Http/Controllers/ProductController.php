...
//        }
    ...
    public function cv_preview(Request $request,$id)
    {
        ...
        if($id == 0){
            $data['design'] = Cv_design::select('id','design_number')->where('user_id',$user->id)->where('cv_id',null)->first();
            ...
            $data['design'] = Cv_design::select('id','design_number')->where('user_id',$user->id)->where('cv_id',$id)->first();
...
//        $pdf = PDF::loadView('webview.app_cv_design.default_design', ['data' => $data]);//    }else{
        ...
        $pdf->save(env('APP_URL') .'/uploads/cvs/'.$num.'.pdf');
